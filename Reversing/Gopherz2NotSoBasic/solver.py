...
class Client:
    ...
    def request(self, host, port, selector, params=None, resolve_links=False):
        return '\n'.join('\t'.join(thing) for thing in self.item(host, int(port), selector, params=params, resolve_links=resolve_links))



if __name__ == '__main__':
    client = Client()
    print(client.request('web.chal.csaw.io', 4333, '/'))
    params = [
        {
            'key': 'host',
            'value': '216.165.2.48',
    ...
    print(client.request('web.chal.csaw.io', 4333, '/run_client', params=params))
    print(client.request('web.chal.csaw.io', 4333, '/run_client', params=params))
